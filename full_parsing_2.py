...
class mTP:
    def parse_two_tables(self, file1, file2, ws):
        ...
        classes = df1.iloc[:, 1].astype(str).str.strip()
        col2 = df2.iloc[3:, 0].astype(str).str.strip()
        ...
        for value in col2:
            ...
            for key in obj_dict.keys():
                if (obj_dict[key] == "Датчик") and ("Точки" in file2):
                # Ищем точное совпадение в начале строки
                    if re.match(r'^' + re.escape(key) + r'\b', value):
                        ...
                        matched = True
                        break
                elif (obj_dict[key] == "Арматура") and ("арматура" in file2):
                    if re.match(r'^' + re.escape(key) + r'\b', value):
                        results.append([key, "Совпадает", "green"])
                        used_keys.add(key)
                        matched = True
